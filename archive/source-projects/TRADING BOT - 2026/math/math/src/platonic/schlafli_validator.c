    ...
    if (V > 0 && E > 0 && F > 0) {
        if (symbol->dimension == 3) {
            ...
            if (!result.euler_valid) {
                snprintf(result.error_message, sizeof(result.error_message),
                         "Euler characteristic %lld != %lld",
                         (long long)result.euler_characteristic, (long long)result.expected_euler);
                return result;
            }
...
void schlafli_print_validation(const SchlafliValidationResult* result) {
    ...
    if (result->euler_characteristic != 0 || result->expected_euler != 0) {
        printf("  Euler Characteristic: %lld %s %lld - %s\n",
               (long long)result->euler_characteristic,
               result->euler_valid ? "==" : "!=",
               (long long)result->expected_euler,
