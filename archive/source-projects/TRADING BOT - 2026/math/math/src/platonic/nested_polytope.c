    ...
    uint32_t current_depth, NestedPolytopeCallback callback, void* user_data)
{
    ...
    if (current_depth == target_depth) {
        ...
        for (uint32_t i = 0; i < node->num_children; i++) {
            traverse_at_depth_impl(node->children[i], target_depth, current_depth + 1, callback, user_data);
        }
    }
}
...
static void update_recursive_coords(NestedPolytopeNode* node) {
    for (uint32_t i = 0; i < node->num_children; i++) {
        nested_polytope_compute_world_coords(node->children[i]);
        update_recursive_coords(node->children[i]);
    }
}
...
bool nested_polytope_update_all_coords(NestedPolytopeTree* tree) {
    ...
    update_recursive_coords(tree->root);
    return true;
}
...
static void count_at_depth_cb(NestedPolytopeNode* node, void* user_data) {
    (void)node;
...
uint32_t nested_polytope_count_at_depth(const NestedPolytopeTree* tree, uint32_t depth) {
    if (!tree) return 0;
    uint32_t count = 0;
    nested_polytope_traverse_level((NestedPolytopeTree*)tree, depth, count_at_depth_cb, &count);
    return count;
}

typedef struct { uint32_t min_d; uint32_t max_d; } LeafDepthCtx;

static void check_leaf_cb(NestedPolytopeNode* node, void* user_data) {
    LeafDepthCtx* ctx = (LeafDepthCtx*)user_data;
    if (node->num_children == 0) {
        if (node->depth < ctx->min_d) ctx->min_d = node->depth;
        if (node->depth > ctx->max_d) ctx->max_d = node->depth;
    }
}

bool nested_polytope_is_balanced(const NestedPolytopeTree* tree) {
    if (!tree || !tree->root) return true;
    LeafDepthCtx ctx = { .min_d = UINT32_MAX, .max_d = 0 };
    nested_polytope_traverse_dfs(tree->root, check_leaf_cb, &ctx);
    return (ctx.max_d - ctx.min_d) <= 1;
}

typedef struct { uint32_t* count; uint32_t max_nodes; NestedPolytopeNode** nodes; } CollectNodesCtx;

static void collect_node_cb(NestedPolytopeNode* node, void* user_data) {
    CollectNodesCtx* ctx = (CollectNodesCtx*)user_data;
    ...
    uint32_t max_nodes)
{
    ...
    uint32_t count = 0;
    CollectNodesCtx ctx = { .count = &count, .max_nodes = max_nodes, .nodes = nodes };
    nested_polytope_traverse_level((NestedPolytopeTree*)tree, depth, collect_node_cb, &ctx);
    return count;
}
...
static void check_symmetry_cb(NestedPolytopeNode* node, void* user_data) {
...
bool nested_polytope_preserves_12fold_symmetry(const NestedPolytopeTree* tree) {
    if (!tree || !tree->root) return false;
    bool preserves = true;
    nested_polytope_traverse_dfs(tree->root, check_symmetry_cb, &preserves);
    return preserves;
}
...
static void validate_node_cb(NestedPolytopeNode* node, void* user_data) {
...
bool nested_polytope_validate_tree(const NestedPolytopeTree* tree) {
    if (!tree || !tree->root) return false;
    bool valid = true;
    nested_polytope_traverse_dfs(tree->root, validate_node_cb, &valid);
    return valid;
}
