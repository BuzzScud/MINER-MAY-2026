...
void discovery_results_print(const DiscoveryResults* results) {
    ...
    for (uint32_t i = 0; i < results->count; i++) {
        ...
        printf("    Dimension: %uD\n", p->dimension);
        printf("    Vertices: %llu, Edges: %llu", (unsigned long long)p->vertices, (unsigned long long)p->edges);
        if (p->dimension >= 3) {
            printf(", Faces: %llu", (unsigned long long)p->faces);
        }
        if (p->dimension >= 4 && p->cells > 0) {
            printf(", Cells: %llu", (unsigned long long)p->cells);
        }
        printf("\n");
