...
function renderCategories() {
    ...
    if (categories.length === 0) {
        container.innerHTML = `
            <div class="col-span-full flex flex-col items-center justify-center py-12 text-center">
                ...
                <p class="text-sm font-medium text-gray-700 mb-1">No categories yet</p>
                <p class="text-xs text-gray-500">Add your first category to get started</p>
            </div>
        `;
        return;
    }
    ...
    container.innerHTML = categories.map(cat => `
        <div class="group relative p-4 bg-gradient-to-br from-gray-50 to-white border-2 border-gray-200 rounded-xl hover:border-blue-300 hover:shadow-md transition-all cursor-pointer">
            <div class="flex items-center gap-3">
                <div class="w-5 h-5 rounded-full shadow-sm" style="background-color: ${cat.color}"></div>
                <span class="text-sm font-semibold text-gray-900 flex-1">${cat.name}</span>
            </div>
            <div class="absolute inset-0 bg-blue-50 opacity-0 group-hover:opacity-100 rounded-xl transition-opacity -z-10"></div>
        </div>
    `).join('');
}
...
let billsViewMode = 'grid';
let billsFiltered = [];

function loadBillsPage() {
    setupBillsFilters();
    billsFiltered = []; // Reset filter to show all bills
    renderBillsCards();
    updateBillsSummary();
...
function setupBillsFilters() {
    ...
    const categoryFilter = document.getElementById('bills-category-filter');
    if (categoryFilter) {
        const allOption = categoryFilter.querySelector('option[value="all"]');
        categoryFilter.innerHTML = allOption ? allOption.outerHTML : '';
        categories.forEach(cat => {
            const option = document.createElement('option');
            option.value = cat.name;
            option.textContent = cat.name;
    ...
    // View toggle
    const gridViewBtn = document.getElementById('bills-view-grid');
    const listViewBtn = document.getElementById('bills-view-list');
    
    if (gridViewBtn) {
        gridViewBtn.addEventListener('click', () => {
            billsViewMode = 'grid';
            gridViewBtn.classList.add('active-view', 'bg-blue-50');
            listViewBtn.classList.remove('active-view', 'bg-blue-50');
            document.getElementById('bills-grid-view').classList.remove('hidden');
            document.getElementById('bills-list-view').classList.add('hidden');
            renderBillsCards();
        });
    }
    ...
    if (listViewBtn) {
        listViewBtn.addEventListener('click', () => {
            billsViewMode = 'list';
            listViewBtn.classList.add('active-view', 'bg-blue-50');
            gridViewBtn.classList.remove('active-view', 'bg-blue-50');
            document.getElementById('bills-grid-view').classList.add('hidden');
            document.getElementById('bills-list-view').classList.remove('hidden');
            renderBillsCards();
        });
    }
}
...
function filterBills() {
    const searchTerm = document.getElementById('bills-search')?.value.toLowerCase() || '';
    const statusFilter = document.getElementById('bills-status-filter')?.value || 'all';
    const categoryFilter = document.getElementById('bills-category-filter')?.value || 'all';
    
    billsFiltered = reminders.filter(reminder => {
        // Search filter
        ...
        const isDueSoon = daysUntil > 0 && daysUntil <= 7 && !reminder.is_paid;
        
        let matchesStatus = true;
        if (statusFilter === 'paid') {
            matchesStatus = reminder.is_paid;
        } else if (statusFilter === 'unpaid') {
            matchesStatus = !reminder.is_paid;
        } else if (statusFilter === 'overdue') {
            matchesStatus = isOverdue;
        } else if (statusFilter === 'due-soon') {
            matchesStatus = isDueSoon || isToday;
        }
...
function updateBillsSummary() {
    ...
    const paid = reminders.filter(r => r.is_paid).length;
    const overdue = reminders.filter(r => {
        const dueDate = new Date(r.due_date);
        const daysUntil = Math.ceil((dueDate - new Date()) / (1000 * 60 * 60 * 24));
        return daysUntil < 0 && !r.is_paid;
    }).length;
    const dueSoon = reminders.filter(r => {
        const dueDate = new Date(r.due_date);
        const daysUntil = Math.ceil((dueDate - new Date()) / (1000 * 60 * 60 * 24));
        return daysUntil >= 0 && daysUntil <= 7 && !r.is_paid;
    }).length;
    
    document.getElementById('bills-total-count').textContent = total;
    document.getElementById('bills-paid-count').textContent = paid;
    document.getElementById('bills-overdue-count').textContent = overdue;
    document.getElementById('bills-due-soon-count').textContent = dueSoon;
}

function renderBillsCards() {
    ...
    // Use filtered bills if available, otherwise use all reminders
    const billsToRender = billsFiltered.length > 0 ? billsFiltered : reminders;
    
    // Sort by due date
    const sortedBills = [...billsToRender].sort((a, b) => {
    ...
    if (sortedBills.length === 0) {
        const emptyState = `
            <div class="col-span-full flex flex-col items-center justify-center py-16 text-center">
                <div class="p-5 bg-gray-100 rounded-full mb-5">
                    <svg class="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                    </svg>
                </div>
                <p class="text-lg font-semibold text-gray-700 mb-2">No bills found</p>
                <p class="text-sm text-gray-500 mb-4">${billsFiltered.length > 0 ? 'Try adjusting your filters' : 'Add your first bill to get started'}</p>
                ${billsFiltered.length > 0 ? '' : `<button onclick="handleAddBillClick(event)" class="py-2 px-4 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium">Add Bill</button>`}
            </div>
        `;
    ...
    // Render grid view
    gridContainer.innerHTML = sortedBills.map(reminder => renderBillCard(reminder, 'grid')).join('');
    
    // Render list view
    listContainer.innerHTML = sortedBills.map(reminder => renderBillCard(reminder, 'list')).join('');
}

function renderBillCard(reminder, view = 'grid') {
        const dueDate = new Date(reminder.due_date);
        const daysUntil = Math.ceil((dueDate - new Date()) / (1000 * 60 * 60 * 24));
        const isOverdue = daysUntil < 0 && !reminder.is_paid;
        const isToday = daysUntil === 0 && !reminder.is_paid;
    ...
    let statusText = daysUntil > 0 ? `${daysUntil} days left` : 'Past Due';
    ...
    const categoryColor = categories.find(c => c.name === reminder.category)?.color || '#6b7280';
    
    if (view === 'list') {
        return `
            <div class="bg-white rounded-xl border-2 ${cardBorderClass} shadow-sm hover:shadow-md transition-all p-5">
                <div class="flex items-start justify-between gap-4">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-3 mb-2">
                            <h3 class="text-lg font-bold text-gray-900 truncate">${reminder.title}</h3>
                            <span class="inline-flex items-center gap-x-1.5 py-1 px-3 rounded-full text-xs font-semibold ${statusBadgeClass} border whitespace-nowrap">
                        ${statusText}
                    </span>
                            ${reminder.recurring ? `<span class="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium bg-indigo-100 text-indigo-700 border border-indigo-300 whitespace-nowrap">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                                ...
                                <p class="text-xl font-bold text-gray-900">$${parseFloat(reminder.amount || 0).toFixed(2)}</p>
                            </div>
                            <div>
                                <p class="text-gray-500 mb-1">Due Date</p>
                                <p class="font-semibold text-gray-900">${dueDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                            </div>
                            <div>
                                <p class="text-gray-500 mb-1">Category</p>
                                <div class="flex items-center gap-2">
                                    <div class="w-3 h-3 rounded-full" style="background-color: ${categoryColor}"></div>
                                    <p class="font-semibold text-gray-900">${reminder.category}</p>
                                </div>
                            </div>
                            <div>
                                <p class="text-gray-500 mb-1">Actions</p>
                                <div class="flex items-center gap-2">
                                    <button onclick="showBillDetails(${reminder.id})" class="text-blue-600 hover:text-blue-800 text-sm font-medium">View</button>
                                    <button onclick="editReminder(${reminder.id})" class="text-blue-600 hover:text-blue-800 text-sm font-medium">Edit</button>
                                    ${!isPaid ? `<button onclick="markReminderPaid(${reminder.id})" class="text-green-600 hover:text-green-800 text-sm font-medium">Mark Paid</button>` : `<button onclick="markReminderUnpaid(${reminder.id})" class="text-orange-600 hover:text-orange-800 text-sm font-medium">Mark Unpaid</button>`}
                                    <button onclick="deleteReminder(${reminder.id})" class="text-red-600 hover:text-red-800 text-sm font-medium">Delete</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="flex flex-col gap-2">
                        ${reminder.website_url ? `<a href="${reminder.website_url}" target="_blank" rel="noopener noreferrer" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg border border-blue-200" title="Visit Website">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path>
                            </svg>
                        </a>` : ''}
                        ${reminder.attachment_url ? `<a href="${reminder.attachment_url}" target="_blank" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg border border-blue-200" title="View Attachment">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                </svg>
                        </a>` : `<input type="file" id="file-upload-${reminder.id}" accept="image/png" class="hidden" onchange="handleFileUpload(${reminder.id}, this.files[0])">
                        <button onclick="document.getElementById('file-upload-${reminder.id}').click()" class="p-2 text-gray-600 hover:bg-gray-50 rounded-lg border border-gray-200" title="Upload Attachment">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    // Grid view
    return `
        <div class="bg-white rounded-xl border-2 ${cardBorderClass} shadow-sm hover:shadow-lg transition-all overflow-hidden group cursor-pointer" onclick="showBillDetails(${reminder.id})">
            <div class="p-5">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex-1 min-w-0">
                        <h3 class="text-lg font-bold text-gray-900 mb-1 truncate group-hover:text-blue-600 transition-colors">${reminder.title}</h3>
                        <div class="flex items-center gap-2 mb-2">
                            <div class="w-2 h-2 rounded-full" style="background-color: ${categoryColor}"></div>
                            <span class="text-sm text-gray-600">${reminder.category}</span>
                        </div>
                    </div>
                    <span class="inline-flex items-center gap-x-1.5 py-1 px-3 rounded-full text-xs font-semibold ${statusBadgeClass} border whitespace-nowrap">
                        ${statusText}
                ...
                <div class="space-y-2 mb-4">
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span class="font-semibold text-gray-900">${dueDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                    </div>
                    ${reminder.recurring ? `
                        <div class="flex items-center gap-2 text-sm">
                            ...
                            <span class="font-semibold text-indigo-700">${reminder.recurrence_type || 'monthly'}</span>
                        </div>
                    ` : ''}
                </div>
                ...
                <div class="flex items-center justify-between pt-4 border-t border-gray-200">
                    <div class="flex items-center gap-2">
                        ${reminder.website_url ? `<a href="${reminder.website_url}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="Visit Website">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path>
                            </svg>
                        </a>` : ''}
                        ${reminder.attachment_url ? `<a href="${reminder.attachment_url}" target="_blank" onclick="event.stopPropagation()" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="View Attachment">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                            </svg>
                        </a>` : `<input type="file" id="file-upload-${reminder.id}" accept="image/png" class="hidden" onchange="handleFileUpload(${reminder.id}, this.files[0])">
                        <button onclick="event.stopPropagation(); document.getElementById('file-upload-${reminder.id}').click()" class="p-1.5 text-gray-600 hover:bg-gray-50 rounded-lg" title="Upload Attachment">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path>
                            </svg>
                        </button>`}
                    ...
                    <div class="flex items-center gap-1">
                        <button onclick="event.stopPropagation(); editReminder(${reminder.id})" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="Edit">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                            </svg>
                        </button>
                        ${!isPaid ? `<button onclick="event.stopPropagation(); markReminderPaid(${reminder.id})" class="p-1.5 text-green-600 hover:bg-green-50 rounded-lg" title="Mark Paid">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                            </svg>
                        </button>` : `<button onclick="event.stopPropagation(); markReminderUnpaid(${reminder.id})" class="p-1.5 text-orange-600 hover:bg-orange-50 rounded-lg" title="Mark Unpaid">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                            </svg>
                        </button>`}
                        <button onclick="event.stopPropagation(); deleteReminder(${reminder.id})" class="p-1.5 text-red-600 hover:bg-red-50 rounded-lg" title="Delete">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function renderReminders() {
    ...
    if (reminders.length === 0) {
        container.innerHTML = `
            <div class="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p class="text-sm font-medium text-gray-700 mb-1">No reminders yet</p>
                <p class="text-xs text-gray-500">Add your first bill reminder to get started</p>
            </div>
        `;
        return;
    }
    ...
    container.innerHTML = reminders.map(reminder => {
        ...
        const isOverdue = dueDate < new Date() && !reminder.is_paid;
        const isToday = dueDate.toDateString() === new Date().toDateString() && !reminder.is_paid;
        const daysUntil = Math.ceil((dueDate - new Date()) / (1000 * 60 * 60 * 24));
        
        let statusClass = 'bg-blue-100 text-blue-800 border-blue-200';
        let statusText = `${daysUntil} days left`;
        ...
        return `
            <div class="group p-4 bg-gradient-to-br from-white to-gray-50 border-2 border-gray-200 rounded-xl hover:border-orange-300 hover:shadow-md transition-all">
                <div class="flex items-start justify-between gap-4">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="w-3 h-3 rounded-full flex-shrink-0" style="background-color: ${categoryColor}"></div>
                            <h3 class="font-bold text-gray-900 truncate">${reminder.title}</h3>
                            <span class="inline-flex items-center gap-x-1.5 py-1 px-2.5 rounded-full text-xs font-semibold ${statusClass} border whitespace-nowrap">
                            ${statusText}
                        </span>
                            ${reminder.recurring ? `
                                <span class="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium bg-indigo-100 text-indigo-700 border border-indigo-300 whitespace-nowrap">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                    </svg>
                                    ${reminder.recurrence_type || 'monthly'}
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                            <div>
                                <p class="text-gray-500 mb-1">Amount</p>
                                <p class="font-semibold text-gray-900">$${parseFloat(reminder.amount || 0).toFixed(2)}</p>
                </div>
                            <div>
                                <p class="text-gray-500 mb-1">Due Date</p>
                    ...
                    ${!reminder.is_paid ? `
                            <button onclick="markReminderPaid(${reminder.id})" class="py-2 px-3 text-xs font-semibold bg-green-600 text-white rounded-lg hover:bg-green-700 shadow-sm hover:shadow transition-all whitespace-nowrap">
                                Mark Paid
                            </button>
                        ` : `
                            <button onclick="markReminderUnpaid(${reminder.id})" class="py-2 px-3 text-xs font-semibold bg-orange-600 text-white rounded-lg hover:bg-orange-700 shadow-sm hover:shadow transition-all whitespace-nowrap">
                                Mark Unpaid
                            </button>
                        `}
                        <button onclick="editReminder(${reminder.id})" class="py-2 px-3 text-xs font-semibold bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-sm hover:shadow transition-all whitespace-nowrap">
                            Edit
                        </button>
                        <button onclick="deleteReminder(${reminder.id})" class="py-2 px-3 text-xs font-semibold bg-red-600 text-white rounded-lg hover:bg-red-700 shadow-sm hover:shadow transition-all whitespace-nowrap">
                            Delete
                        </button>
                    </div>
                </div>
...
async function markReminderUnpaid(id) {
    try {
        const response = await fetch(`${API_BASE}/reminders/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        <div class="space-y-6">
            ...
            <div class="pt-4 border-t border-gray-200">
                <div class="flex gap-3">
                    ...
                    ` : `
                        <button onclick="markReminderUnpaid(${reminder.id}); closeBillDetailsModal();" class="flex-1 py-2.5 px-4 inline-flex items-center justify-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-orange-600 text-white hover:bg-orange-700">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                            </svg>
                            Mark as Unpaid
...
window.markReminderUnpaid = markReminderUnpaid;
