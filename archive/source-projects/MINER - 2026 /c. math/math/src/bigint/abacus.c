...
MathError abacus_div(CrystallineAbacus* quotient, CrystallineAbacus* remainder,
    ...
    /* Binary search with guard to avoid non-termination (e.g. on Mac) */
    const size_t max_div_iterations = 256;
    size_t div_iterations = 0;
    while (abacus_compare(low, high) <= 0) {
        if (++div_iterations > max_div_iterations) {
            abacus_free(abs_a);
            abacus_free(abs_b);
            abacus_free(low);
            abacus_free(high);
