 ...
 * Recursion depth is bounded by SPHERE_HIERARCHY_MAX_DEPTH to avoid unbounded
 * allocation (node count is 1 + 12 + ... + 12^max_depth).
 */
...
/** Maximum allowed hierarchy depth to prevent exponential memory use. */
#define SPHERE_HIERARCHY_MAX_DEPTH 10
 ...
 * max_depth is clamped to SPHERE_HIERARCHY_MAX_DEPTH to avoid unbounded allocation.
 */
CompactSphere* create_sphere_hierarchy(uint32_t max_depth) {
    if (max_depth > SPHERE_HIERARCHY_MAX_DEPTH) {
        max_depth = SPHERE_HIERARCHY_MAX_DEPTH;
    }
    ...
    /* Recursion bounded by SPHERE_HIERARCHY_MAX_DEPTH to avoid memory explosion. */
...
/**
 * @brief Recursively create child spheres (depth bounded by SPHERE_HIERARCHY_MAX_DEPTH).
 */
