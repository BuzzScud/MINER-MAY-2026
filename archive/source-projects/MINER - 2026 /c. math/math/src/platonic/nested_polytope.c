...
/* Context structs for callbacks (replacing nested function closures) */
typedef struct { uint32_t min_depth; uint32_t max_depth; } LeafDepthContext;
typedef struct {
...
static void traverse_at_depth_impl(NestedPolytopeNode* node, uint32_t target_depth,
    uint32_t current_depth, NestedPolytopeCallback callback, void* user_data) {
    if (!node) return;
    if (current_depth == target_depth) {
        callback(node, user_data);
    } else if (current_depth < target_depth) {
        for (uint32_t i = 0; i < node->num_children; i++) {
            traverse_at_depth_impl(node->children[i], target_depth, current_depth + 1,
                callback, user_data);
        }
    }
}

static void check_leaf_callback(NestedPolytopeNode* node, void* user_data) {
    LeafDepthContext* ctx = (LeafDepthContext*)user_data;
    if (node->num_children == 0) {
        if (node->depth < ctx->min_depth) ctx->min_depth = node->depth;
        if (node->depth > ctx->max_depth) ctx->max_depth = node->depth;
    }
}

static void collect_node_callback(NestedPolytopeNode* node, void* user_data) {
    CollectNodeContext* ctx = (CollectNodeContext*)user_data;
    if (ctx->count < ctx->max_nodes) {
...
{
    if (!tree || !tree->root || !callback) return;
    traverse_at_depth_impl(tree->root, depth, 0, callback, user_data);
}
...
static void update_recursive_impl(NestedPolytopeNode* node) {
    for (uint32_t i = 0; i < node->num_children; i++) {
        nested_polytope_compute_world_coords(node->children[i]);
        update_recursive_impl(node->children[i]);
    }
}
...
bool nested_polytope_update_all_coords(NestedPolytopeTree* tree) {
    ...
    update_recursive_impl(tree->root);
    return true;
}
...
static void count_at_depth_callback(NestedPolytopeNode* node, void* user_data) {
    (void)node;
...
uint32_t nested_polytope_count_at_depth(const NestedPolytopeTree* tree, uint32_t depth) {
    if (!tree) return 0;
    uint32_t count = 0;
    nested_polytope_traverse_level((NestedPolytopeTree*)tree, depth, count_at_depth_callback, &count);
    return count;
}

bool nested_polytope_is_balanced(const NestedPolytopeTree* tree) {
    if (!tree || !tree->root) return true;
    LeafDepthContext ctx = { .min_depth = UINT32_MAX, .max_depth = 0 };
    nested_polytope_traverse_dfs(tree->root, check_leaf_callback, &ctx);
    return (ctx.max_depth - ctx.min_depth) <= 1;
}
    ...
    uint32_t max_nodes)
{
    ...
    CollectNodeContext ctx = { .nodes = nodes, .max_nodes = max_nodes, .count = 0 };
    nested_polytope_traverse_level((NestedPolytopeTree*)tree, depth, collect_node_callback, &ctx);
    return ctx.count;
}
...
static void check_symmetry_callback(NestedPolytopeNode* node, void* user_data) {
...
bool nested_polytope_preserves_12fold_symmetry(const NestedPolytopeTree* tree) {
    if (!tree || !tree->root) return false;
    bool preserves = true;
    nested_polytope_traverse_dfs(tree->root, check_symmetry_callback, &preserves);
    return preserves;
}
...
static void validate_node_callback_static(NestedPolytopeNode* node, void* user_data) {
...
bool nested_polytope_validate_tree(const NestedPolytopeTree* tree) {
    if (!tree || !tree->root) return false;
    bool valid = true;
    nested_polytope_traverse_dfs(tree->root, validate_node_callback_static, &valid);
    return valid;
}
