 ...
 *
 * max_depth is clamped to an implementation-defined maximum (see sphere_hopping.c
 * SPHERE_HIERARCHY_MAX_DEPTH, typically 10) to avoid unbounded allocation.
 * Callers must not rely on depths larger than that maximum.
 *
 * @param max_depth Maximum recursion depth (clamped internally)
