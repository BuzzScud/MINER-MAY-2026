...
/**
 * @brief Generate nonce using unified pipeline (clock lattice primary, SFT fallback)
 * @param config Nonce configuration
 * @param result Output result
 * @param abacus88d_ctx Optional Abacus88D* for storing tetration (NULL to skip)
 * @return true on success
 */
bool nonce_generate_unified(const NonceConfig* config, NonceResult* result, void* abacus88d_ctx);

/**
 * @brief Select 88D layer from tetration value magnitude
 */
uint8_t nonce_select_layer(uint64_t tetration_value);

/**
 * @brief Select 88D dimension from block height (clock position 1-11)
 */
