...
#define _DARWIN_C_SOURCE 1  /* for sysctl types (u_int, etc.) */
#endif
...
int detect_num_cpu_cores(void) {
    ...
    /* macOS: avoid sysctl (C11 headers). Use sysconf if available, else fallback. */
    ...
    if (sysctl(mib, 2, &nprocs, &len, NULL, 0) == 0 && nprocs > 0) {
        return nprocs;
    }
    return 1;
#else
