...
#ifdef __APPLE__
/* pthread_barrier implementation for macOS */
int pthread_barrier_init(pthread_barrier_t* b, const void* attr, unsigned count) {
    (void)attr;
    if (!b || count == 0) return -1;
    b->total = b->count = count;
    b->phase = 0;
    return pthread_mutex_init(&b->mutex, NULL) ? -1 : (pthread_cond_init(&b->cond, NULL) ? -1 : 0);
}
int pthread_barrier_wait(pthread_barrier_t* b) {
    ...
    while (phase == b->phase) pthread_cond_wait(&b->cond, &b->mutex);
    pthread_mutex_unlock(&b->mutex);
    return 0;
}
