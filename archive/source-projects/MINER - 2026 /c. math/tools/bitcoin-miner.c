...
typedef struct {
    ...
    bool use_nonce_deterministic;
    bool use_legacy;  /* true = linear search only; false = unified pipeline (default) */
} mining_config_t;
...
static HierarchicalThreadPool* g_88d_pool = NULL;
...
void print_usage(const char* program_name) {
    ...
    printf("  --nonce-deterministic     Use deterministic nonce (SFT + tetration)\n");
    printf("  --all-algorithms          Enable all optimizations\n");
    printf("  --unified                 Unified pipeline: clock+SFT+tetration+recovery+platonic (default)\n");
    printf("  --legacy                  Linear nonce search only (benchmarking)\n\n");
    ...
    printf("  --nonce-deterministic: Deterministic nonce from block height + difficulty\n");
    printf("                  Uses SFT + tetration (Crystalline); try base ± offset\n\n");
 ...
 * Output is standard SHA256 order (big-endian).
 */
void double_sha256(const uint8_t* data, size_t len, uint8_t* hash) {
    uint8_t temp[SHA256_DIGEST_LENGTH];
#ifdef USE_MINIMAL_SHA256
    sha256_minimal(data, len, temp);
    sha256_minimal(temp, SHA256_DIGEST_LENGTH, hash);
#else
...
/**
 * Convert compact 'bits' to 32-byte big-endian target (Bitcoin consensus)
 */
static void bits_to_target_bytes(uint32_t bits, uint8_t target[32]) {
    memset(target, 0, 32);
    uint8_t exponent = (uint8_t)(bits >> 24);
    uint32_t mantissa = bits & 0x00FFFFFF;
    if (exponent <= 3) {
        uint32_t val = mantissa >> (8 * (3 - exponent));
        target[28] = (val >> 24) & 0xFF;
        target[29] = (val >> 16) & 0xFF;
        target[30] = (val >> 8) & 0xFF;
        target[31] = val & 0xFF;
    } else {
        int byte_off = 32 - 3 - (exponent - 3);
        if (byte_off >= 0 && byte_off < 30) {
            target[byte_off] = (mantissa >> 16) & 0xFF;
            target[byte_off + 1] = (mantissa >> 8) & 0xFF;
            target[byte_off + 2] = mantissa & 0xFF;
        }
    }
}

/**
 * Compare 32-byte hash to target (both big-endian). Returns true if hash <= target.
 */
static bool hash_meets_target(const uint8_t* hash, const uint8_t* target) {
    for (int i = 0; i < 32; i++) {
...
/**
 * Convert compact 'bits' to difficulty_bits (1-64) for nonce entropy scaling.
 * nonce_apply_difficulty_bounds expects 1-64; raw bits would skip entropy reduction.
 * Matches Python bits_compact_to_difficulty_bits: exponent from high byte, clamped.
 */
static uint32_t bits_compact_to_difficulty_bits(uint32_t bits) {
...
/**
 * Check if hash meets difficulty target (Bitcoin consensus)
 */
bool check_hash_difficulty(const uint8_t* hash, uint32_t bits) {
    uint8_t target[32];
    bits_to_target_bytes(bits, target);
...
/**
 * Unified mining: uses all thesis nodes together.
 * 1. nonce_generate_unified (clock lattice + SFT + tetration) for base nonce
 * 2. Recovery (OBJECTIVE 28) for suggested region
 * 3. Platonic ordering (12 vertices, golden ratio) around base
 * 4. Real double-SHA256 + target check
 */
static bool try_nonce_range(uint8_t block_header[80], uint32_t* nonces, size_t count,
                            uint32_t start_nonce, uint32_t end_nonce,
                            block_template_t* template, uint8_t hash[32]) {
    for (size_t i = 0; i < count && !g_shutdown; i++) {
...
bool mine_block_unified(block_template_t* template, uint32_t start_nonce, uint32_t end_nonce) {
    uint8_t block_header[80];
    uint8_t hash[32];
    memcpy(block_header, &template->version, 4);
    memcpy(block_header + 4, template->prev_block, 32);
    ...
    /* Step 1: Abacus88D + base nonce from unified generation */
    Abacus88D* abacus = abacus88d_create(60);
    NonceConfig nonce_config;
    uint32_t difficulty_bits = bits_compact_to_difficulty_bits(template->bits);
    nonce_config_init(&nonce_config, (uint64_t)template->height, difficulty_bits);
    NonceResult nonce_result;
    uint32_t base_nonce = start_nonce;
    if (nonce_generate_unified(&nonce_config, &nonce_result, (void*)abacus)) {
        base_nonce = (uint32_t)(nonce_result.nonce & 0xFFFFFFFF);
        if (base_nonce < start_nonce || base_nonce >= end_nonce) {
            base_nonce = start_nonce + (end_nonce - start_nonce) / 2;
        }
    }

    /* Step 2: Recovery (OBJECTIVE 28) suggested region */
    uint32_t recovery_nonce = base_nonce;
    recovery_config_t rec_cfg = recovery_default_config();
    rec_cfg.max_iterations = 500;
    rec_cfg.method = RECOVERY_METHOD_CRYPTO;
    recovery_context_t* rec_ctx = recovery_init(&rec_cfg);
    if (rec_ctx) {
        uint8_t target_bits[4];
        memcpy(target_bits, &template->bits, 4);
        if (recovery_set_q(rec_ctx, target_bits, 4) == RECOVERY_OK) {
            if (recovery_run(rec_ctx) == RECOVERY_OK) {
                recovery_result_t* res = recovery_get_result(rec_ctx);
                if (res && res->converged && res->length >= 4) {
                    memcpy(&recovery_nonce, res->data, 4);
                    if (recovery_nonce < start_nonce || recovery_nonce >= end_nonce) {
                        recovery_nonce = base_nonce;
                    }
                    recovery_free_result(res);
                }
            }
        }
        recovery_free(rec_ctx);
    }

    /* Step 3: Sphere hopping (create_sphere_hierarchy) + Duality (fold_to_q1, is_prime_position) */
    CompactSphere* root = create_sphere_hierarchy(2);
    const double phi = 1.618033988749895;
    const int platonic_radius = 500;
    const uint32_t nonce_range = end_nonce - start_nonce;
    const uint32_t sphere_size = (nonce_range / 12) > 0 ? (nonce_range / 12) : 1;
    uint32_t candidates[1024];
    size_t n = 0;
    candidates[n++] = base_nonce;
    candidates[n++] = recovery_nonce;
    /* Process 12 spheres in duality-folded order (Q1 first); prime positions get more steps */
    if (root) {
        for (int s = 0; s < 12 && n < 1024; s++) {
            DualityClockPosition pos = clock_position_create((uint8_t)s);
            ...
            if (root->children[s]) {
                for (int i = 0; i < steps && n < 1024; i++) {
                    uint32_t offset = (uint32_t)(i * phi) % platonic_radius;
                    if (sphere_base + offset < end_nonce) candidates[n++] = sphere_base + offset;
                    if (offset > 0 && sphere_base >= offset) candidates[n++] = sphere_base - offset;
                }
            } else {
                for (int i = 0; i < 35 && n < 1024; i++) {
                    uint32_t offset = (uint32_t)(i * phi) % platonic_radius;
                    if (sphere_base + offset < end_nonce) candidates[n++] = sphere_base + offset;
                    if (offset > 0 && sphere_base >= offset) candidates[n++] = sphere_base - offset;
                }
    ...
    /* Base and recovery neighborhoods */
    for (int off = -200; off <= 200 && n < 1024; off++) {
        candidates[n++] = base_nonce + off;
        candidates[n++] = recovery_nonce + off;
    }

    if (abacus) abacus88d_free(abacus);

    if (try_nonce_range(block_header, candidates, n, start_nonce, end_nonce, template, hash)) {
        return true;
    }
...
/**
 * Quadrant partition for 4 threads: full 32-bit space, thesis ch.15 unbiased.
 * Each thread tries only nonces with clock_quadrant_from_nonce(nonce) == thread_id.
 */
static bool mine_block_unified_quadrant(block_template_t* template, int thread_id) {
    const uint64_t nonce_range_full = 0xFFFFFFFFULL + 1;  /* full 32-bit space */
    uint8_t block_header[80];
    uint8_t hash[32];
    memcpy(block_header, &template->version, 4);
    memcpy(block_header + 4, template->prev_block, 32);
    ...
    const uint32_t sphere_size = (uint32_t)(nonce_range_full / 12);
    uint32_t candidates[1024];
    size_t n = 0;
    candidates[n++] = base_nonce;
    candidates[n++] = recovery_nonce;
    ...
    const double phi = 1.618033988749895;
    const int platonic_radius = 500;
    if (root && sphere_size > 0) {
        for (int s = 0; s < 12 && n < 1024; s++) {
            uint32_t sphere_base = (uint32_t)((uint64_t)s * sphere_size % nonce_range_full);
            bool prime_pos = is_prime_position((uint8_t)s);
            int steps = prime_pos ? 40 : 35;
            for (int i = 0; i < steps && n < 1024; i++) {
                uint32_t offset = (uint32_t)(i * phi) % platonic_radius;
                uint64_t v = (uint64_t)sphere_base + offset;
                if (v <= 0xFFFFFFFFULL) candidates[n++] = (uint32_t)v;
                if (offset > 0 && sphere_base >= offset) candidates[n++] = sphere_base - offset;
            }
        }
        free_sphere_hierarchy(root);
    ...
    /* Try candidates in this quadrant only */
    for (size_t i = 0; i < n && !g_shutdown; i++) {
        uint32_t nonce = candidates[i];
        if (clock_quadrant_from_nonce(nonce) != (uint8_t)thread_id) continue;
        memcpy(block_header + 76, &nonce, 4);
        double_sha256(block_header, 80, hash);
        pthread_mutex_lock(&g_stats_mutex);
        g_stats.hashes_computed++;
    ...
    /* Linear sweep: only nonces in our quadrant, skip already-tried candidates */
    for (uint64_t nn = 0; nn <= 0xFFFFFFFFULL; nn++) {
        if (g_shutdown) break;
        uint32_t nonce = (uint32_t)nn;
        if (clock_quadrant_from_nonce(nonce) != (uint8_t)thread_id) continue;
        bool skip = false;
        for (size_t i = 0; i < n; i++) {
            if (candidates[i] == nonce) { skip = true; break; }
        }
        if (skip) continue;
        memcpy(block_header + 76, &nonce, 4);
        double_sha256(block_header, 80, hash);
...
/**
 * Deterministic nonce mining (legacy - now part of unified)
 * Uses nonce_generate_deterministic() for base nonce, then tries base ± offset.
 */
bool mine_block_nonce_deterministic(block_template_t* template, uint32_t start_nonce, uint32_t end_nonce) {
    NonceConfig nonce_config;
    uint32_t difficulty_bits = bits_compact_to_difficulty_bits(template->bits);
    nonce_config_init(&nonce_config, (uint64_t)template->height, difficulty_bits);
    NonceResult nonce_result;
    ...
    for (int offset = -offset_range; offset <= offset_range && !g_shutdown; offset++) {
        uint32_t nonce = base_nonce + (uint32_t)offset;
        if (nonce < start_nonce || nonce >= end_nonce) continue;
        memcpy(block_header + 76, &nonce, 4);
        double_sha256(block_header, 80, hash);
        pthread_mutex_lock(&g_stats_mutex);
...
void* mining_thread(void* arg) {
    ...
    while (!g_shutdown) {
        ...
        /* Nonce range: 4 threads = quadrant partition (500D lattice); else 88D or linear */
        uint32_t start_nonce, end_nonce;
        bool found = false;
        if (!g_config.use_legacy && g_config.num_threads == 4) {
            /* Quadrant partition: full 32-bit space, thesis ch.15 unbiased; each thread = one quadrant */
            found = mine_block_unified_quadrant(&template, thread_id);
        } else if (!g_config.use_legacy && g_88d_pool) {
            /* 88D mapping: each thread handles multiple of 88 slots */
            for (uint32_t idx = thread_id; idx < 88 && !g_shutdown; idx += g_config.num_threads) {
                start_nonce = (uint32_t)((uint64_t)idx * 0xFFFFFFFFULL / 88);
                end_nonce = (uint32_t)((uint64_t)(idx + 1) * 0xFFFFFFFFULL / 88);
                found = mine_block_unified(&template, start_nonce, end_nonce);
                if (found) break;
            }
        } else {
            ...
            found = g_config.use_legacy
                ? mine_block_standard(&template, start_nonce, end_nonce)
                : mine_block_unified(&template, start_nonce, end_nonce);
        }
...
void* stats_thread(void* arg) {
    ...
    while (!g_shutdown) {
        ...
        if (elapsed > 0) {
            ...
            /* Display stats */
            const char* algo = g_config.use_legacy ? "LEGACY" : "UNIFIED";
            
            printf("\r[%s] Hashrate: %.2f H/s | Hashes: %" PRIu64 " | Blocks: %" PRIu64 " | Uptime: %lds",
                   algo,
                   hashrate,
                   current_hashes,
                   g_stats.blocks_found,
...
int main(int argc, char** argv) {
    ...
    g_config.use_nonce_deterministic = false;
    g_config.use_legacy = false;  /* Default: unified pipeline */
    g_config.verbose = 0;
    ...
    static struct option long_options[] = {
        ...
        {"nonce-deterministic", no_argument, 0, 'd'},
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "sn:H:p:u:P:a:t:clodAULvhV", long_options, NULL)) != -1) {
        switch (opt) {
            case 's':
                g_config.solo_mining = true;
                break;
            ...
            case 'o':
                ...
                break;
            case 'd':
                g_config.use_nonce_deterministic = true;
                break;
            case 'A':
                g_config.use_crystalline = true;
                g_config.use_platonic = true;
                g_config.use_objective28 = true;
                g_config.use_nonce_deterministic = true;
                g_config.use_legacy = false;
                break;
            case 'U':
                g_config.use_legacy = false;
                break;
            case 'L':
                g_config.use_legacy = true;
                break;
            case 'v':
                g_config.verbose++;
                break;
    ...
    printf("\nMining Mode: %s\n", g_config.use_legacy ? "LEGACY (linear)" : "UNIFIED (all thesis nodes)");
    ...
    /* Create 88D thread pool for unified mode (Option A: structure for 88D mapping) */
    if (!g_config.use_legacy) {
        g_88d_pool = hierarchical_thread_pool_create_88d(60);
        if (!g_88d_pool && g_config.verbose) {
            fprintf(stderr, "Note: 88D pool creation failed, using linear split\n");
        }
    }
    ...
    if (!g_config.solo_mining) {
        printf("Shares Submitted: %" PRIu64 "\n", g_stats.shares_submitted);
    }
    ...
    if (g_stats.blocks_found > 0) {
        printf("\n✓ Successfully mined %" PRIu64 " block(s)!\n", g_stats.blocks_found);
    }
    ...
    // Cleanup
    if (g_88d_pool) {
        hierarchical_thread_pool_free(g_88d_pool);
        g_88d_pool = NULL;
    }
    free(g_config.network);
    free(g_config.rpc_host);
