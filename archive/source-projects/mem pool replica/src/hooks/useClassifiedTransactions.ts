...
import type { FullTransaction } from '../api/mempool';

export type TransactionFilter = 'consolidation' | 'coinjoin' | 'data';

function classifyTransaction(tx: FullTransaction): TransactionFilter | null {
  ...
  const hasOpReturn = tx.vout?.some(
    (o) =>
      ...
      o.scriptpubkey_type?.toLowerCase().includes('op_return')
  );
...
async function fetchAndClassify(filter: TransactionFilter) {
  const recent = await fetchMempoolRecent();
  const results: Array<{ txid: string; vsize: number; fee: number; feePerVsize: number }> = [];
...
export function useClassifiedTransactions(filter: TransactionFilter | null) {
  return useQuery({
