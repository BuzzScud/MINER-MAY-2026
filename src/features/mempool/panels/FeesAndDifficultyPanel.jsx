...
const FEE_TIERS = [
  { key: 'minimumFee', label: 'No Priority', color: 'text-green-400' },
  { key: 'economyFee', label: 'Low Priority', color: 'text-green-500' },
  { key: 'hourFee', label: 'Medium Priority', color: 'text-green-600' },
  { key: 'fastestFee', label: 'High Priority', color: 'text-lime-400' },
];

const GREEN_SEGMENT_PERCENT = 6;

function formatUsd(satPerVb, btcPriceUsd) {
...
export function FeesAndDifficultyPanel() {
  const [activeTab, setActiveTab] = useState('halving');
  ...
  return (
    <Panel title="Transaction Fees & Difficulty Adjustment">
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <section>
          <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-black dark:text-gray-400">
            Transaction Fees
          </h3>
          {feesLoading || !fees ? (
            <div className="flex animate-pulse flex-col gap-3">
              {FEE_TIERS.map((_, i) => (
                <div key={i} className="h-12 rounded bg-gray-200 dark:bg-gray-700/50" />
              ))}
            </div>
          ) : (
              ...
              {FEE_TIERS.map(({ key, label, color }) => {
                ...
                return (
                  <div key={key} className="flex flex-col">
                    ...
                    <span className="text-2xl font-bold text-gray-900 dark:text-white">{satPerVb} sat/vB</span>
        ...
        <section>
          ...
          {difficultyLoading || !difficultyData ? (
            <div className="flex animate-pulse flex-col gap-3">
              <div className="h-2 w-full rounded bg-gray-200 dark:bg-gray-700/50" />
              <div className="h-6 w-32 rounded bg-gray-200 dark:bg-gray-700/50" />
            </div>
          ) : (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm text-black dark:text-gray-400">View</span>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <p className="text-xs text-black dark:text-gray-400">Avg block time</p>
                </div>
                <div>
                  <p
