...
function formatUsd(sats, btcPriceUsd) {
  const btc = sats / 1e8;
...
export function RecentActivityPanel({ onOpenTransaction }) {
  ...
  return (
    <Panel title="Recent Activity">
      <div className="mb-3 flex items-center gap-1 border-b border-gray-200 dark:border-gray-700 pb-2">
        <button
          type="button"
          ...
          className={
            activeTab === 'replacements'
              ? 'text-sky-400'
              : 'text-black dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'
          }
        >
          Recent Replacements
        </button>
          ...
          {replacementsError && (
            <div className="py-4">
              ...
              <button
                ...
                className="mt-2 rounded bg-gray-200 dark:bg-gray-700 px-3 py-1.5 text-sm text-gray-800 dark:text-gray-200 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-500 disabled:opacity-50"
              >
          ...
          {!replacementsError && replacementsLoading && (
            <div className="animate-pulse space-y-2">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-10 rounded bg-gray-200/50 dark:bg-gray-700/50" />
              ))}
            </div>
          )}
          {!replacementsError && !replacementsLoading && (
            <>
              {(!replacements || replacements.length === 0) ? (
                <div className="flex flex-col items-center justify-center py-6 text-center">
                  <p className="text-sm text-black dark:text-gray-400">No RBF replacements right now.</p>
                  <a
                    href={FULLRBF_URL}
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  <button
                    type="button"
                    onClick={() => refetchReplacements()}
                    className="mt-3 rounded bg-gray-200 dark:bg-gray-700 px-3 py-1.5 text-sm text-gray-800 dark:text-gray-200 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-500"
                  >
                    Retry
                  </button>
                </div>
                ...
                <>
                  <div className="mb-2 flex items-center justify-end">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-black dark:text-gray-500 hover:text-gray-400"
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200 dark:border-gray-700 text-left text-gray-900 dark:text-white">
                      ...
                      <tbody>
                        {replacements.slice(0, 10).map((row) => (
                          <tr
                            key={row.txid}
                            className="border-b border-gray-200 dark:border-gray-700/50 text-black dark:text-gray-200"
                          >
      ...
      {activeTab === 'incoming' && (
        <>
          ...
          {(!mempoolError || mempoolStats) && (
            <>
              {connected && (
                <span className="mb-2 inline-block text-xs text-green-600 dark:text-green-400">Live</span>
              )}
              <div className="mb-4 flex flex-wrap items-center gap-4">
                <div>
                  <span className="text-sm text-black dark:text-gray-400">Minimum fee:</span>
                  <span className="ml-2 text-sm font-medium text-blue-400">{minFee} sat/vB</span>
                </div>
                <div className="min-w-[120px] flex-1">
                  <span className="text-sm text-black dark:text-gray-400">Memory Usage:</span>
                ...
                <div>
                  <span className="text-sm text-black dark:text-gray-400">Unconfirmed:</span>
              ...
              <div className="h-[200px] w-full">
                {!connected && incomingChartSeries.length === 0 ? (
                  <div className="flex h-full items-center justify-center rounded bg-gray-200/50 dark:bg-gray-700/30 text-black dark:text-gray-400">
