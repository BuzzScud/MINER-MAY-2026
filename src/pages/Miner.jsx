...
function Metric({ label, value, mono = false }) {
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 p-2 flex flex-col justify-center">
      <div className="text-xs font-semibold uppercase tracking-wider text-black dark:text-gray-400">{label}</div>
  ...
  return (
    <div className="w-full max-w-[1800px] mx-auto px-4">
      ...
      <header className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">BTC Miner</h1>
          <p className="text-sm text-gray-600 dark:text-gray-400">Connect to Bitcoin Core. Use Detect Bitcoin Core to find and connect automatically.</p>
        </div>
        <div
      ...
      <div className="flex gap-0 border-b border-gray-200 dark:border-gray-700 mb-4">
        <button
          ...
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === 'dashboard' ? 'border-blue-600 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}
        >
          Dashboard
        </button>
        <button
          ...
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === 'mempool' ? 'border-blue-600 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}
        >
          Mempool
        </button>
        <button
          ...
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === 'backtest' ? 'border-blue-600 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}`}
        >
          Backtest
        </button>
      </div>
      ...
      {activeTab === 'dashboard' && (
        <>
          {detectMessage && (
            <div className={`mb-4 rounded-lg p-3 ${detectMessage.type === 'success' ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-800 dark:text-green-300' : 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-800 dark:text-red-300'}`}>
              {detectMessage.text}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="flex flex-col gap-4">
              <Panel title="Connection">
                <p className="text-sm text-black dark:text-gray-400 mb-2 flex items-center gap-2">
                  <span
                ...
                <p className="text-xs text-black dark:text-gray-400 mb-3">{summaryText}</p>
              ...
              <Panel title="Mining">
                ...
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  <select
                    ...
                    className="appearance-none rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm pl-3 pr-8 py-1.5 min-h-[34px] cursor-pointer bg-no-repeat bg-[length:1rem] bg-[right_0.4rem_center] focus:outline-none focus:ring-2 focus:ring-blue-500"
                  ...
                  <button type="button" onClick={() => setWalletModalOpen(true)} className="rounded bg-gray-200 dark:bg-gray-700 px-3 py-1.5 text-sm text-gray-800 dark:text-gray-200 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-500">
                    Edit address
                  </button>
                  <button type="button" onClick={handleStart} className="rounded bg-blue-600 px-3 py-1.5 text-sm text-white hover:bg-blue-700">
                    Start
                  </button>
                ...
                {miningError && <p className="text-sm text-red-400 mt-2">{miningError}</p>}
              </Panel>
      ...
      {activeTab === 'backtest' && (
        <Panel title="Backtest (Last N Blocks)">
          <p className="text-xs text-black dark:text-gray-400 mb-3">Test the miner thesis nonce logic on the last N blocks. Uses current connection settings.</p>
          <div className="flex flex-wrap items-center gap-2 mb-3">
            <label className="text-sm text-black dark:text-gray-400">
              Blocks to test
              <input
                type="number"
                min={1}
                ...
                className="ml-2 w-20 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-2 py-1 text-sm"
              />
            </label>
            <button type="button" onClick={handleBacktest} disabled={backtestRunning} className="rounded bg-blue-600 px-3 py-1.5 text-sm text-white hover:bg-blue-700 disabled:opacity-50">
              {backtestRunning ? 'Running…' : 'Run backtest'}
            </button>
          </div>
          {backtestStatus && <p className="text-sm text-black dark:text-gray-400 mb-2">{backtestStatus}</p>}
          {backtestResult?.error && <p className="text-sm text-red-400">{backtestResult.error}</p>}
          {backtestResult?.ok && (
      ...
      {connectionModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setConnectionModalOpen(false)}>
          <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 max-w-md w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Connection settings</h2>
            <div className="space-y-2 mb-3">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Network
                <select value={config.network} onChange={(e) => setConfig((c) => ({ ...c, network: e.target.value, rpc_port: DEFAULT_PORTS[e.target.value] ?? c.rpc_port }))} className="mt-1 block w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-2 py-1.5 text-sm">
              ...
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Host
                <input type="text" value={config.rpc_host} onChange={(e) => setConfig((c) => ({ ...c, rpc_host: e.target.value }))} className="mt-1 block w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-2 py-1.5 text-sm" />
              </label>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Port
                <input type="number" value={config.rpc_port} onChange={(e) => setConfig((c) => ({ ...c, rpc_port: Number(e.target.value) || DEFAULT_PORTS[c.network] }))} className="mt-1 block w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-2 py-1.5 text-sm" />
              </label>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                User (optional)
                <input type="text" value={config.rpc_user} onChange={(e) => setConfig((c) => ({ ...c, rpc_user: e.target.value }))} placeholder="Optional" className="mt-1 block w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-2 py-1.5 text-sm" />
              </label>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Password (optional)
                <input type="password" value={config.rpc_password} onChange={(e) => setConfig((c) => ({ ...c, rpc_password: e.target.value }))} placeholder="Optional" className="mt-1 block w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-2 py-1.5 text-sm" />
              </label>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Cookie file (optional)
                <input type="text" value={config.cookie_file} onChange={(e) => setConfig((c) => ({ ...c, cookie_file: e.target.value }))} placeholder="e.g. /path/to/.cookie" className="mt-1 block w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-2 py-1.5 text-sm" />
              </label>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Data folder (optional)
                <input type="text" value={config.datadir} onChange={(e) => setConfig((c) => ({ ...c, datadir: e.target.value }))} placeholder="e.g. ~/Library/Application Support/Bitcoin" className="mt-1 block w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-2 py-1.5 text-sm" />
              </label>
            </div>
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setConnectionModalOpen(false)} className="rounded bg-gray-200 dark:bg-gray-700 px-3 py-1.5 text-sm text-gray-800 dark:text-gray-200">
                Close
              </button>
              <button type="button" onClick={() => { checkConnection(); setConnectionModalOpen(false); }} className="rounded bg-blue-600 px-3 py-1.5 text-sm text-white hover:bg-blue-700">
                Connect
              </button>
            </div>
          </div>
      ...
      {walletModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setWalletModalOpen(false)}>
          <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 max-w-md w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Mining payout address</h2>
            <input
              type="text"
              value={config.mining_address}
              onChange={(e) => setConfig((c) => ({ ...c, mining_address: e.target.value }))}
              placeholder="bc1q... or bcrt1q..."
              className="block w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-2 py-1.5 text-sm mb-3"
            />
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setWalletModalOpen(false)} className="rounded bg-gray-200 dark:bg-gray-700 px-3 py-1.5 text-sm text-gray-800 dark:text-gray-200">
                Cancel
              </button>
              <button type="button" onClick={saveAddress} className="rounded bg-blue-600 px-3 py-1.5 text-sm text-white hover:bg-blue-700">
                Save
              </button>
            </div>
          </div>
